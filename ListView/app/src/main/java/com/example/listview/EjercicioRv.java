...
public class EjercicioRv extends AppCompatActivity {
    ...
    public boolean onOptionsItemSelected(@NonNull MenuItem item) {
        int posicion = miAdaptador.getSelectedPos();
        ab = getSupportActionBar();
        if (miAdaptador.getSelectedPos() != RecyclerView.NO_POSITION){
            if (item.getItemId() == R.id.añadir){
                sistemas.add(posicion, new SistemaOperativo("prueba: " + posicion, String.valueOf(posicion),R.drawable.new_item));
                miAdaptador.notifyItemInserted(posicion);

                miAdaptador.notifyItemChanged(posicion + 1);
                ...
                sistemas.get(posicion).setNombre("Modificado");

                miAdaptador.notifyItemChanged(posicion);
            } else if (item.getItemId() == R.id.mover){
                SistemaOperativo aux = sistemas.get(posicion);
                int newPos = 0;
                ...
                miAdaptador.notifyItemChanged(posicion);
                miAdaptador.notifyItemMoved(posicion, newPos);
